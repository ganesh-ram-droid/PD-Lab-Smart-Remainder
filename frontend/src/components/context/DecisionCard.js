...
const DecisionCard = ({ decision, score }) => {
  return (
    <View className="rounded-3xl bg-white p-4 shadow-sm" style={{ shadowColor: '#0F172A' }}>
      ...
      <Text className="mt-1 text-sm text-slate-500">How the system will handle the reminder</Text>
      <View className="mt-4 flex-row items-center justify-between rounded-2xl bg-slate-50 px-4 py-4">
        <View className="flex-1 pr-3">
          ...
          <Text className="mt-1 text-sm font-semibold text-slate-950">
            {Number(score || 0) >= 70 ? 'Will Notify' : decision || 'Suppressed'}
          </Text>
        </View>
        <DecisionBadge decision={Number(score || 0) >= 70 ? 'Will Notify' : decision} />
      </View>
    </View>
  );
};
